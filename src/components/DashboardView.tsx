...
import { CheckCircle2, XCircle, AlertTriangle, Calendar, ChevronRight, UserPlus, BookOpen, Clock, HeartHandshake, CalendarOff } from 'lucide-react';
...
interface DashboardViewProps {
  ...
  onLogAttendance: (subjectId: string, status: 'present' | 'absent') => void;
      ...
      <main className="flex flex-col lg:flex-row gap-12 lg:items-stretch">
        ...
        <section className="lg:w-1/2 flex flex-col gap-8">
          ...
          <div className="bg-white dark:bg-bunk-card-dark rounded-[20px] p-6 sm:p-8 border border-bunk-border-light dark:border-zinc-800 shadow-sm flex-1 flex flex-col justify-between">
            <div className="flex justify-between items-end mb-8 border-b border-dashed border-bunk-border-light dark:border-zinc-800 pb-3">
              <h3 className="text-[10px] font-bold uppercase tracking-[0.15em] text-bunk-sub-light dark:text-zinc-400">
                Today's Schedule
              </h3>
              <span className="text-[10px] tracking-widest uppercase font-mono text-zinc-500 dark:text-zinc-400 font-bold">
            ...
            <div className="space-y-6">
              {todayClasses.length === 0 ? (
                ...
                todayClasses.map((cls) => {
                  ...
                  const isDone = logs.some(log => log.subjectId === cls.subjectId && log.date === new Date().toISOString().split('T')[0]);
                        ...
                        {!isDone && (
                          <div className="flex gap-1.5 sm:gap-2 shrink-0 items-center">
                            {isCancelled ? (
                              ...
                              <>
                                <button
                                  onClick={() => onLogAttendance(cls.subjectId, 'present')}
                                ...
                                <button
                                  onClick={() => onLogAttendance(cls.subjectId, 'absent')}
